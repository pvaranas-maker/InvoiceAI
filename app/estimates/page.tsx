...
export default function InvoicesPage() {
    ...
    function getEstimateStatus(estimate: SavedInvoice | null) {
        if (!estimate) return status || "Draft";
    ...
    async function saveEstimate() {
        ...
        const response = await fetch("/api/estimates", {
            ...
            body: JSON.stringify({
                estimateNumber: `EST-${nextInvoiceNumber}`,
        ...
        setInvoices([newEstimate, ...invoices]);
    ...
    async function convertEstimateToInvoice() {
        if (!selectedInvoice) {
            alert("Please select a saved estimate first.");
            return;
        }
    ...
    function generateInvoicePDF() {
        ...
        doc.text(
            `Estimate #: ${selectedInvoice?.invoiceNumber ?? nextInvoiceNumber}`,
            140,
            32
        );
        ...
        doc.setFontSize(16);
        doc.text(`Estimated Total:&nbsp;{formatCurrency(total)}`, 140, finalY + 32);
    ...
    function downloadPDF() {
        ...
        doc.setFontSize(16);
        doc.text(`Estimated Total: $${total.toFixed(2)}`, 140, finalY + 32);

        doc.save(`Estimate-${nextInvoiceNumber}.pdf`);
    }
                ...
                <section className="flex-1 py-12 px-6">
                    <div className="max-w-6xl mx-auto">
                        ...
                        <div className="text-center mb-12">
                            ...
                            <h1 className="text-4xl md:text-5xl font-extrabold mt-3 mb-3 tracking-tight">New Estimate</h1>
                            <p className="text-lg text-gray-400 max-w-xl mx-auto">
                                Create a professional estimate for your customer in minutes.
                            </p>
                        </div>
                        ...
                        <div className="grid lg:grid-cols-12 gap-10 items-start">
                            ...
                            <div className="lg:col-span-7 space-y-6">
                                <div className="bg-[#12151f] border border-gray-800/80 rounded-2xl p-6 md:p-8 shadow-xl">
                                    ...
                                    <div className="space-y-4">
                                        ...
                                        <div className="flex flex-wrap gap-3 pt-2">
                                            ...
                                            >
                                                {selectedInvoice ? "Update Estimate" : "Save Estimate"}
                                            </button>
                                            ...
                                            {selectedInvoice && (
                                                <button
                                                    ...
                                                    className="rounded-xl bg-emerald-600 hover:bg-emerald-700 transition-colors px-6 py-3 font-semibold"
                                                >
                                                    Convert to Invoice
                                                </button>
                                            )}
                                ...
                                {/* Saved Estimates List — restyled to match the dark card system, sits below the form on this side */}
                                <div className="bg-[#12151f] border border-gray-800/80 rounded-2xl p-6 md:p-8 shadow-xl">
                                    <h2 className="text-xl font-semibold mb-6 text-gray-200">Saved Estimates</h2>

                                    {invoices.length === 0 ? (
                                        <p className="text-gray-500 text-sm">No estimates saved yet.</p>
                                    ) : (
                                        <div className="space-y-3">
                                            {invoices.map((invoice, index) => (
                                                <div
                                                    key={`${invoice.id}-${index}-${invoice.createdAt}`}
                                                    onClick={async () => {

                                                        const res = await fetch(`/api/estimates/${invoice.id}`);
                                                        const fullInvoice = await res.json();

                                                        setSelectedInvoice(fullInvoice);
                                                        setStatus(getEstimateStatus(fullInvoice || null));
                                                        setDueDate(
                                                            fullInvoice?.validUntil
                                                                ? fullInvoice.validUntil.substring(0, 10)
                                                                : ""
                                                        );
                                                ...
                                                >
                                                    <div>
                                                        ...
                                                        <p className="text-xs text-gray-500 mt-0.5">
                                                            Estimate #{invoice.invoiceNumber} • {invoice.createdAt}
                                                        </p>

                                                        <div className="mt-2">
                                                            <span
                                                                className={`text-xs px-2.5 py-1 rounded-full font-semibold uppercase tracking-wider ${statusBadgeClasses(getEstimateStatus(invoice))}`}
                                                            >
                                                                {getEstimateStatus(invoice)}
                                                            </span>
                                                        </div>
                                                    </div>
                                ...
                                <div className="p-6 md:p-8">
                                    <div className="flex justify-between items-start mb-8">
                                        ...
                                        <div className="text-right text-xs text-slate-500">
                                            ...
                                            {dueDate && <div className="text-purple-600 font-medium mt-1">Valid Through: {dueDate}</div>}
                                        </div>
                                    </div>

                                    <div className="mb-6">
                                        <div className="text-xs font-bold text-slate-400 uppercase tracking-wide mb-1">Prepared For</div>
            ...
            {showEmailModal && (
                <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/70 px-4">
                    <div className="w-full max-w-lg bg-[#12151f] border border-gray-800/80 rounded-2xl p-6 md:p-8 shadow-2xl space-y-4">
                        <h2 className="text-xl font-semibold text-gray-200">Email Estimate</h2>
